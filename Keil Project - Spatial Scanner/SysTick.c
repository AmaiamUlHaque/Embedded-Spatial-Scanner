...
void SysTick_WaitMinimum(uint32_t delay){
  ...
  for(i=0; i<delay; i++){
    //SysTick_Wait(1200000);  // wait 10ms (assumes 120 MHz clock)
		SysTick_Wait(24000);  // wait 1.5ms (assumes 120 MHz clock)
		//24k for 1.5ms for 16MHz
...
// 172500 is the number for smallest delay for motor to spin assuming 120MHz (found number via trial and error)
// = 172500 / 120'000'000
// = 0.0014375s
// = 1.4375ms

//for simplicity in this project, used 1.5ms --> waiting 10ms per motor step became very time consuming
